...
def insert_region_asthma(es, bulker):       
    ...
    document_count = es.count(index=RAINFALL_MELBOURNE)['count']

    if(document_count > 0):
        return f'{RAINFALL_MELBOURNE} already has data. Please delete before attempting re-insertion.'

    try:
        with open('../userfunc/deployarchive/data/bom_historic_data/rainfall_cities/Melbourne.csv', 'r') as csv_file:
        ...
        bulker.bulk(es, inserts, index=RAINFALL_MELBOURNE)
